...
import com.example.mx.repository.ProvinciaRepository;
...
public class ProvinciaController {
	...
	/*
	 * @PostMapping("/provincias") public ResponseEntity<Provincia>
	 * createProvincias(@RequestBody Provincia padre) {
	 * 
	 * <<<<<<< HEAD List<Provincia> page = provinciaRepository.findAll();
	 * page.size(); System.out.println("id "+page.size()); if (padre.getId() !=
	 * null) { return ResponseEntity.badRequest() .header("Content-Type",
	 * "application/json") .body(null); } long idnueva = (page.size() + 1);
	 * padre.setId(idnueva); Provincia result = provinciaRepository.save(padre);
	 * result.setId(idnueva); return ResponseEntity.ok() .header("Content-Type",
	 * "application/json").body(result); ======= >>>>>>>
	 * b6ffd4751903239fc012b53b9d6a940e68ee4b0a
	 * 
	 * Provincia result = provinciaRepository.save(padre); return
	 * ResponseEntity.ok() .header("Content-Type", "application/json").body(result);
	 * }
	 */
	...
	public ResponseEntity<String> updateProvincias(@RequestBody Provincia provincia, @PathVariable Long id)
			throws ResourceNotFoundException {

		Provincia encontrarProvincia = getProvincias(id)
